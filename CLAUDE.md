...
- `elements/chunks/yasmartcaptcha.tpl` — чанк `tpl.YaSmartCaptcha` (`<div class="smart-captcha" data-sitekey="[[+client_key]]">`).
...
`enabled`, `service_js`, `client_key`, `server_key`, `send_user_ip`, `fail_open`. При `enabled = false` хук всегда возвращает `true`, рендер — пустую строку. При пустом `server_key` проверка всегда проваливается (с записью в лог).
